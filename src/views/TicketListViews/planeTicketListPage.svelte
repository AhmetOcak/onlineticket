...
<script>
    ...
    onMount(async () => {
        try {
            searchResult = await (await axios.get(`https://onlineticketbackendapi.azure-api.net/v1/api/Plane_Travels/${$searchRequest.departurePlace}/${$searchRequest.arrivalPlace}`)).data;
